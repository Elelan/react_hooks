...
const countLettersInWords = (word) => {
    let i = 0;
    while (i < 3000000) i++;
    return word.length;
};
...
export default () => {
    ...
    const letterCount = useMemo(() => countLettersInWords(currentWord), [currentWord]);

    // const letterCount = countLettersInWord(processWord);
    ...
    return (
        <section>
            ...
            <p>
                <button onClick={handleLoadNextWord}>load next word</button>
                <button onClick={() => setCount(count + 1)}>notch up the count</button>
            </p>
            <hr/>
            <p>
                "{currentWord}" has {letterCount} letters. This took <strong>{computedTime} seconds</strong> to process
            </p>
            <p>Current word is still "{currentWord}", but
                we've {count === 0 ? 'not increased the count yet' : `increased the count ${count} times`}</p>
        </section>
    );
}